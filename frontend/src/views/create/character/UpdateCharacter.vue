...
const route = useRoute() //取id
const characterId = route.params.character_id//取对应角色的id，在路由里是下划线id在这里也是下划线id
const character = ref(null)//character需要在前端里显示所以是响应式变量

onMounted(async () => {//当组件被挂起完后取出组件中对应的虚拟角色
  try {
...
async function handleUpdate() {
  ...
  if (!photo) {
    ...
    formData.append('character_id', characterId)//更新角色需要传一个id
