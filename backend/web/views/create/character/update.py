...
class UpdateCharacterView(APIView):
    ...
    def post(self, request):
        try:
            character_id = request.data['character_id']#传一个角色id，django
            character = Character.objects.get(id=character_id, author__user=request.user)#获取一下这个角色，满足条件id匹配角色匹配
            name = request.data['name'].strip()#读取用户名
